...
export default function OnboardingPage() {
  ...
  const canProceed = () => {
    switch (currentStep) {
      ...
      case 1:
        return projects.some(p => p.trim().length > 0);
      case 2:
    ...
    try {
      const { error } = await supabase
        ...
        .upsert({
          ...
          youtube: youtube.trim() ? `https://youtube.com/@${youtube.trim()}` : null,
          tiktok: tiktok.trim() ? `https://tiktok.com/@${tiktok.trim()}` : null,
                ...
                {projects.map((project, index) => (
                  <div key={index} className="flex items-center gap-2">
                    <input
                      ...
                      placeholder={index === 0 ? "e.g., A marketplace for local artisans" : "e.g., An AI writing assistant"}
                      className="flex-1 px-4 py-3 text-base border border-input rounded-lg outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all bg-background"
                      autoFocus={index === 0}
                    />
                    {speechSupported && (
                      <button
                        ...
                        title={listeningIndex === index ? "Stop recording" : "Voice input"}
                      >
                        <svg
                          className={`w-5 h-5 ${listeningIndex === index ? "animate-pulse" : ""}`}
                          fill="none"
                          viewBox="0 0 24 24"
                          stroke="currentColor"
                          strokeWidth={2}
                    ...
                    {projects.length > 1 && (
                      <button
                        ...
                        className="shrink-0 w-10 h-10 rounded-lg flex items-center justify-center border border-input text-muted-foreground hover:text-red-500 hover:border-red-300 transition-all"
            ...
            {currentStep === 4 && (
              <div className="space-y-3">
                ...
                <div>
                  ...
                  <div className="flex items-center border border-input rounded-lg overflow-hidden focus-within:border-primary focus-within:ring-2 focus-within:ring-primary/20 transition-all bg-background">
                    <div className="flex items-center gap-2 pl-3 pr-1 text-muted-foreground shrink-0">
